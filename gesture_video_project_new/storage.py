...
class DescriptionStorage:
    ...
    # def get_description_by_time(self, target_datetime):
    #     conn = sqlite3.connect(self.db_path, check_same_thread=False)
    #     try:
    #         c = conn.cursor()
    #         c.execute('''SELECT description, timestamp 
    #                    FROM descriptions 
    #                    ORDER BY ABS(STRFTIME('%s', timestamp) - STRFTIME('%s', ?)) 
    #                    LIMIT 1''', (target_datetime,))
    #         result = c.fetchone()
    ...
    def get_description_by_date(self, target_date_str):
        conn = sqlite3.connect(self.db_path, check_same_thread=False)
        try:
            c = conn.cursor()
            c.execute('''SELECT description, timestamp 
                       ...
                       LIMIT 1''', (target_date_str,))
            result = c.fetchone()
            return result if result else (None, None)
        except Exception as e:
            logging.error(f"Error retrieving description by date: {e}")
            return None, None
        finally:
            conn.close()
