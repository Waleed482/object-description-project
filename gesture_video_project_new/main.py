...
def speak_welcome_message(speech_queue):
    ...
    I can help you understand your surroundings. 'describe' to hear a description, 
   """
...
def main():
    ...
    global is_assistance_enabled
    global frame_count
    global prev_time
    global last_report_time
    global has_asked_stop_question # Declare as global within the function if modifying
    ...
    has_asked_stop_question = False  # Initialize the flag here
    last_stop_time = 0
    stop_cooldown = 5
    waiting_for_date = False # Initialize waiting_for_date

    date_query_timeout = 5  # Seconds to listen for date input
    ...
    try:
        while True:
            ...
            # Handle rock gesture for report
            waiting_for_date = False
            if "rock" in current_gesture.lower():
                print("________inside rock block________")
                current_time = time.time()
            ...
            elif "stop" in current_gesture.lower():
                print("________inside stop block________")
                if not has_asked_stop_question:
                    speech_queue.add_speech("Which date's description would you like to know?")
                    has_asked_stop_question = False  # Mark that the question has been asked
                    last_stop_time = time.time() # Initialize last_stop_time when question is asked

            # elif "stop" in current_gesture.lower():
            #     print("________inside stop block________")
            #     if not has_asked_stop_question:
            #         speech_queue.add_speech("Which date's description would you like to know?")
            #         has_asked_stop_question = True
            #         waiting_for_date = True
            #         date_query_start_time = time.time()
            #         print("________listening for date...________")
            #         speech_handler.start_listening_for_date() # Assuming you have a method to start specific listening

            # # Listen for date after stop gesture
            # if waiting_for_date and date_query_start_time:
            #     elapsed_time = time.time() - date_query_start_time
            #     if elapsed_time >= date_query_timeout:
            #         print("________date listening timed out________")
            #         speech_queue.add_speech("Sorry, I didn't hear a date. Resuming normal operation.")
            #         waiting_for_date = False
            #         speech_handler.stop_listening() # Assuming you have a method to stop specific listening
            #     else:
            #         spoken_date = speech_handler.check_for_date() # Assuming this checks if a date was spoken (e.g., "9 april")
            #         if spoken_date:
            #             print(f"________date received: {spoken_date}________")
            #             try:
            #                 parts = spoken_date.lower().split()
            #                 if len(parts) == 2 and parts[0].isdigit() and parts[1] in calendar.month_name[1:]:
            #                     day = int(parts[0])
            #                     month_name = parts[1]
            #                     month_number = list(calendar.month_name).index(month_name.capitalize())
            #                     year = datetime.now().year # Assume current year for simplicity
            #                     target_date_str = f"{year:04d}-{month_number:02d}-{day:02d}"
            #                     print(f"________formatted date: {target_date_str}________")
            #                     description, timestamp = description_storage.get_description_by_date(target_date_str)
            #                     if description:
            #                         speech_queue.add_speech(f"Description from {datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S').strftime('%B %d')}: {description}")
            #                     else:
            #                         speech_queue.add_speech(f"No description found for {datetime.strptime(target_date_str, '%Y-%m-%d').strftime('%B %d')}.")
            #                 else:
            #                     speech_queue.add_speech("Sorry, I didn't understand the date format. Please try again like '9 April'.")
            #             except ValueError:
            #                 speech_queue.add_speech("Sorry, I couldn't understand the date. Please try again.")
            #             finally:
            #                 waiting_for_date = False
            ...
            #     # Check if a user has given a command (e.g., "What happened on 9th April")
            #     user_question = speech_handler.check_for_command()
            #     if user_question:
            #         try:
            #             # Parse the date from the query (e.g., "What happened on 9th April")
            #             parts = user_question.lower().split()
            #             month_str = ""
            #             day_int = None
            #             for part in parts:
            #                 if part in speech_handler.month_names:
            #                     month_str = part
            #                     break
            #             for part in parts:
            #                 try:
            ...
            #             # If month and day are found, process the date query
            #             if month_str and day_int:
            #                 year = datetime.now().year
            #                 target_date_str = f"{year}-{speech_handler.month_names[month_str]}-{day_int:02d}"
            #                 description_data = DescriptionStorage.get_description_by_date(target_date_str)

            #                 if description_data:
            #                     description, timestamp = description_data
            #                     try:
            #                         date_obj = datetime.strptime(timestamp.split()[0], "%Y-%m-%d")
            #                         speech_queue.add_speech(f"On {date_obj.strftime('%B %d')}, I recorded: {description}")
            #                     except ValueError:
            #                         speech_queue.add_speech(f"On {target_date_str}, I recorded: {description}")
            #                 else:
            #                     speech_queue.add_speech(f"No description found for {month_str.capitalize()} {day_int}.")
            #             else:
            #                 speech_queue.add_speech("Sorry, I couldn't understand the date. Please try again.")
            #         except Exception as e:
            #             logger.error(f"Date query processing error: {e}")
            #             speech_queue.add_speech("Sorry, I encountered an error processing the date.")
                    
            #         # Reset the waiting flag after processing the date query
            #         waiting_for_date = False
            ...
            if is_assistance_enabled:
                ...
                # report_command = speech_handler.check_for_report_command()
                # if report_command:
                #     speech_handler.pause_listening()
                #     latest_description, timestamp = description_storage.get_latest_description()
                #     if latest_description:
                #         speech_queue.add_speech(f"Here is the latest report from {timestamp}: {latest_description}")
                #     else:
                #         speech_queue.add_speech("No previous descriptions are available.")
                #     speech_handler.resume_listening()
